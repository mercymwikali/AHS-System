Table 85167 "FLT-Travel Requisition Staff"
{
    fields
    {
        field(50000; "Req No"; Code[20])
        {
        }
        field(50001; "Employee No"; Code[20])
        {
            TableRelation = "HR-Employee"."No.";

            trigger OnValidate()
            begin
                if HrEmployee.Get("Employee No") then begin
                    "Employee Name" := HrEmployee."First Name" + ' ' + HrEmployee."Middle Name" + ' ' + HrEmployee."Last Name";
                    Position := HrEmployee.Initials;
                end;
            end;
        }
        field(50002; "Employee Name"; Text[70])
        {
        }
        field(50003; Position; Text[70])
        {
        }
        field(50004; "Daily Work Ticket"; Code[20])
        {
        }
        field(50005; EntryNo; Integer)
        {
            AutoIncrement = true;
        }
    }

    keys
    {
        key(Key1; "Req No", EntryNo)
        {
            Clustered = true;
        }
    }

    fieldgroups
    {
    }

    var
        HrEmployee: Record "HR-Employee";
}
