Report 52202600 "Deductions Summary 2"
{
    DefaultLayout = RDLC;
    RDLCLayout = './Layouts/DeductionsSummary2.rdl';

    dataset
    {
        dataitem("prPeriod Transactions"; "prPeriod Transactions")
        {
            DataItemTableView = sorting("Transaction Code", "Payroll Period", Membership, "Reference No") where("Group Order" = filter(8));
            ...
            column(mcompanyName; COMPANYNAME)
            {
            }
            column(idno; idno)
            {
            ...
            begin
                if PrevTran <> "prPeriod Transactions"."Transaction Code" then begin
                    PrevTran := "prPeriod Transactions"."Transaction Code";
                    Counter := 0;
                end;
                Counter := Counter + 1;
                ...
                countz := countz + 1;
                Addr1 := '';
                if prTransCode.Get("prPeriod Transactions"."Transaction Code") then
                    Addr1 := prTransCode.Address;
                ...
                Clear(empName);
                if emps.Get("prPeriod Transactions"."Employee Code") then begin
