Codeunit 52202439 prPayrollProcessing
{
    trigger OnRun()
    begin
    end;
    ...
    var
        ...
        EmployeePostingGroup: Record "prEmployee Posting Group";
        ...
        SecondarySHA: Decimal;
        selectedPp: Text;
        SHATaxable: Decimal;
        AHLTaxable: Decimal;
        NssfTierI: Decimal;
        NssfTierII: Decimal;
        TotalNssfContribution: Decimal;
        TierOneAmount: Decimal;
        TierTwoAmount: Decimal;
        periods: Record "prPayroll Periods";
        LocumTransactionCode: Code[10];
    ...
    begin
        ...
        intSHIF_BasedOn := VitalSetup."SHIF Based on";
        ...
        ImplementHousingLevy := VitalSetup."Implement Housing Levy";
        EmployeeHousingPerc := VitalSetup."Employee Housing Levy";
        EmployerHousingPerc := VitalSetup."Employer Housing Levy";
        LocumTransactionCode := VitalSetup."Locum Transaction Code";
        Clear(PhDisabled);
        Clear(curAllRecRel);
    end;

    procedure fnProcesspayroll(strEmpCode: Code[30]; dtDOE: Date; curBasicPay: Decimal; blnPaysPaye: Boolean; blnPaysNssf: Boolean; blnPaysShif: Boolean; SelectedPeriod: Date; dtOpenPeriod: Date; Membership: Text[30]; ReferenceNo: Text[30]; dtTermination: Date; blnGetsPAYERelief: Boolean; Dept: Code[20]; GratuityPerc: Decimal; blnPaysSHA: Boolean; blnPaysAHL: Boolean; blnpaysTierOne: Boolean; BlnTierTwo: Boolean)
    var
        ...
        SpecialTransType: Option Ignore,"Defined Contribution","Home Ownership Savings Plan","Life Insurance","Owner Occupier Interest","Prescribed Benefit","Salary Arrears","Staff Loan","Value of Quarters",Morgage,Gratuity,"Insurance Relief","Allowance Recovery";
        TGroup: Text[30];
    ...
    begin
        ...
        if HrEmployee.Find('-') then begin
            // if (HrEmployee."Contract End Date" <> 0D) and (HrEmployee."Contract End Date" < Today) then
            // error('The contract for Employee No. ' + strEmpCode + ' expired on ' + format(HrEmployee."Contract End Date"));

            PhDisabled := HrEmployee."Physical Disability";
            curBasicPay := curBasicPay;
            if HrEmployee."Contract Type" = HrEmployee."Contract Type"::Locum then begin
                prEmployeeTransactions.Reset();
                prEmployeeTransactions.SetRange("Employee Code", HrEmployee."No.");
                prEmployeeTransactions.SetRange("Transaction Code", LocumTransactionCode);
                prEmployeeTransactions.SetRange("Payroll Period", SelectedPeriod);
                if prEmployeeTransactions.FindFirst() then begin
                    curBasicPay := prEmployeeTransactions.Amount;
                end;
            end;
        end;
        // Message('Employee %1 Salary %2', strEmpCode, curBasicPay);
        blnPaysPaye := false;
        EmpSalary.Reset();
        EmpSalary.SetRange(EmpSalary."Employee Code", strEmpCode);
        if EmpSalary.find('-') then
            blnPaysPaye := EmpSalary."Pays PAYE";
        //Initialize
        selectedPp := Format(SelectedPeriod);

        EmployeePostingGroup.Reset();
        EmployeePostingGroup.SetRange(EmployeePostingGroup.Code, 'SALARY');
        if EmployeePostingGroup.Find('-') then begin
            ...
            if (curBasicPay >= 0) then begin
                ...
                TGroupOrder := 1;
                TSubGroupOrder := 1;
                salariesAcc := EmployeePostingGroup."Salary Account";
                ...
                //Insert Gratuity
                IF GratuityPerc > 0 THEN
                    fnUpdateEmployerDeductions(strEmpCode, 'GRAT',
                     'EMP', TGroupOrder, TSubGroupOrder, 'Gratuity', curTransAmount * (GratuityPerc / 100), 0, intMonth, intYear,
                      '', '', SelectedPeriod);
                ...
                if prSalaryArrears.Find('-') then
                    repeat
                        ...
                        TGroupOrder := 7;
                        TSubGroupOrder := 4;
                        TaxAccount := EmployeePostingGroup."Income Tax Account";
                ...
                // prEmployeeTransactions.SetFilter("Transaction Code", '<>%1', VitalSetup."Locum Transaction Code");
                if prEmployeeTransactions.Find('-') then begin
                    curTotAllowances := 0;
                    if prEmployeeTransactions."Transaction Code" <> LocumTransactionCode then begin
                        repeat
                            ...
                            if prTransactionCodes.Find('-') then begin
                                if prTransactionCodes."Transaction Code" <> LocumTransactionCode then begin
                                    ...
                                    IF prTransactionCodes.Exception = TRUE THEN
                                        curException := curException + curTransAmount;
                                    //Prorate Basic Pay on    {What if someone leaves within the same month they are employed}
                                    if dtTermination <> 0D then
                                        if (Date2dmy(dtTermination, 2) = Date2dmy(dtOpenPeriod, 2)) and (Date2dmy(dtTermination, 3) = Date2dmy(dtOpenPeriod, 3)) then begin
                                            CountDaysofMonth := fnDaysInMonth(dtTermination);
                                    ...
                                    IF prTransactionCodes."Non Cash Benefit" = FALSE THEN
                                    ...
                                    IF prTransactionCodes."Non Cash Benefit" = FALSE THEN
                                        ...
                                        , '', CoopParameters::none)
                                    ELSE
                                        curNonCash := curNonCash + curTransAmount;
                                end;
                            end;
                ...
                //Get the NSSF amount
                //NEW NSSF CODE HERE
                //Get the N.S.S.F amount for the montfnGetEmployerNSSFh GBT
                ...
                //Tier 1
                IF blnPaysNssf THEN BEGIN //Pays NSSF
                    IF blnpaysTierOne THEN BEGIN
                        TierOneAmount := 0;
                        curNSSF := fnGetEmployeeNSSF(curNssf_Base_Amount, 1);
                        TierOneAmount := ROUND(curNSSF);
                        curNSSF := ROUND(curNSSF, 1, '=');
                        curTransAmount := curNSSF;
                        strTransDescription := 'N.S.S.F TIER 1';
                        TGroup := 'STATUTORIES';
                        TGroupOrder := 7;
                        TSubGroupOrder := 1;
                        ReferenceNo := 'nnsf ' + Format(SelectedPeriod);

                        // Message('Employee Code %1 Tier 1: %2 Baseamount: %3', strEmpCode, curTransAmount, curNssf_Base_Amount);

                        fnUpdatePeriodTrans(strEmpCode, 'NSSF Tier 1', TGroup, TGroupOrder, TSubGroupOrder, strTransDescription, curTransAmount, 0, intMonth, intYear, Membership, ReferenceNo, SelectedPeriod, Dept,
               NSSFEMPyee, Journalpostas::Credit, Journalpostingtype::"G/L Account", '', Coopparameters::NSSF);
                    end;
                    IF BlnTierTwo THEN BEGIN
                        TierTwoAmount := 0;
                        curNSSF := fnGetEmployeeNSSF(curNssf_Base_Amount, 2);
                        curNSSF := ROUND(curNSSF);
                        TierTwoAmount := curNSSF;
                        curTransAmount := curNSSF;
                        ...
                        // Message('Employee Code %1, NssfBase:%3 Tier 2: %2', strEmpCode, curTransAmount, curNssf_Base_Amount);

                        fnUpdatePeriodTrans(strEmpCode, 'NSSF Tier 2', TGroup, TGroupOrder,
                        TSubGroupOrder, strTransDescription, curTransAmount, 0, intMonth, intYear, Membership, ReferenceNo, SelectedPeriod, Dept,
                       NSSFEMPyee, Journalpostas::Credit, Journalpostingtype::"G/L Account", '', Coopparameters::NSSF);

                    end;
                    // Matching Employer Contribution
                    curNSSF := TierOneAmount + TierTwoAmount;
                    curTransAmount := TierOneAmount + TierTwoAmount;
                ...
                END;
                curDefinedContrib := 0;
                IF blnPaysNssf = TRUE THEN
                    curDefinedContrib := TierOneAmount + TierTwoAmount
                ELSE
                    curDefinedContrib := 0;
                ...
                if ImplementHousingLevy then begin
                    ...
                    if EmployeeHousingPerc <> 0 then
                        EmployeeHousingLevy := ROUND(curGrossPay * (EmployeeHousingPerc / 100));
                    ...
                    if EmployerHousingPerc <> 0 then
                        EmployerHousingLevy := ROUND(curGrossPay * (EmployerHousingPerc / 100));
                ...
                // curPensionStaff := curPensionStaff + curNSSF;

                curDefinedContrib := TierOneAmount + TierTwoAmount;//(curPensionStaff + curNonTaxable) - curMorgageReliefAmount;
                curTransAmount := curDefinedContrib;
                strTransDescription := 'Defined Contributions';
                TGroup := 'TAX CALCULATIONS';
                TGroupOrder := 6;
                ...
                curGrossTaxable := curGrossPay + curBenefits + curValueOfQuarters + curNonCash;
                ...
                curPensionStaff := 0;
                curNSSF := fnGetEmployerNSSF(curNssf_Base_Amount, 2) - curNssfEmployee;
                ...
                IF VitalSetup."Implement SHIF Relief" THEN BEGIN
                    VitalSetup.TESTFIELD("SHIF Relief Percentage");

                    IF blnPaysShif THEN BEGIN

                        IF intSHIF_BasedOn = intSHIF_BasedOn::Gross THEN
                            curShif_Base_Amount := curGrossPay;
                        IF intSHIF_BasedOn = intSHIF_BasedOn::Basic THEN
                            curShif_Base_Amount := curBasicPay;

                        curSHIF := fnGetEmployeeSHIF(curShif_Base_Amount);
                        if curShif_Base_Amount > MinTaxablePay then
                            currNHIFRelief := curSHIF * (VitalSetup."SHIF Relief Percentage" / 100);

                        //ERROR('NHIF is %1 and the relief is %2',currNHIFRelief,curNHIF);

                        curShif_Base_Amount := 0;
                        curSHIF := 0;

                        IF currNHIFRelief > 0 THEN BEGIN
                            curTransAmount := ROUND(currNHIFRelief, 1, '<');
                ...
                if blnPaysPaye then begin
                    ...
                    if HrEmployee.Get(strEmpCode) then
                        // if HrEmployee."Employee Type" = HrEmployee."Employee Type"::Secondary then begin
                        //     VitalSetup.Get();
                        //     SecondarySHA := 0;
                        //     if ImplementHousingLevy then
                        //         SecondarySHA := Round(curGrossTaxable * (VitalSetup."AHL Percentage" / 100));

                        //     // curTaxablePay := curGrossTaxable - SecondarySha;
                        //     curTaxCharged := (curTaxablePay) * 0.325;
                        //     curTransAmount := curTaxCharged;
                        //     curTransAmount := ROUND(curTaxCharged, 0.01, '>');
                        //     strTransDescription := 'Tax Charged';
                        //     TGroup := 'TAX CALCULATIONS';
                        //     TGroupOrder := 6;
                        //     TSubGroupOrder := 7;

                        //     fnUpdatePeriodTrans(strEmpCode, 'TXCHRG', TGroup, TGroupOrder, TSubGroupOrder, strTransDescription, curTransAmount, 0, intMonth, intYear, '', '', SelectedPeriod, Dept, '', JournalPostAs::Credit, JournalPostingType::" ", '', CoopParameters::none);
                        // end else begin
                    ...
                    if curInsuranceReliefAmount > 0 then begin
                        IF curInsuranceReliefAmount >= curMaximumRelief THEN
                ...
                IF curNonCash > 0 THEN BEGIN
                    strTransDescription := 'Non-Cash Benefits';
                    TGroup := 'TAX CALCULATIONS';
                    TGroupOrder := 6;
                    TSubGroupOrder := 5;
                    fnUpdatePeriodTrans(strEmpCode, 'NONCASH', TGroup, TGroupOrder, TSubGroupOrder, strTransDescription,
                    curNonCash, 0, intMonth, intYear, '', '', SelectedPeriod, Dept, '', JournalPostAs::" ", JournalPostingType::" "
                    , '', CoopParameters::none);
                END;
                ...
                SHATaxable := 0;
                AHLTaxable := 0;
                IF (blnPaysAHL = TRUE) THEN
                    AHLTaxable := ROUND(curGrossTaxable * (VitalSetup."Employee Housing Levy" / 100)) ELSE
                    AHLTaxable := 0;
                IF (blnPaysSHA = TRUE) THEN
                    SHATaxable := ROUND(curGrossTaxable * (VitalSetup."SHA Deduction Percentage" / 100)) ELSE
                    SHATaxable := 0;
                IF blnPaysNssf = FALSE THEN BEGIN
                    NssfTierI := 0;
                    NssfTierII := 0
                END;
                //end ****************** CALCULATE SHA and AHL to reduce taxable pay****************
                // IF blnPaysPaye = FALSE THEN BEGIN
                //     SHATaxable := 0;
                //     AHLTaxable := 0;
                // END;
                //END SHIF CHANGES
                ...
                if curPensionStaff > curMaxPensionContrib then begin
                    if (PhDisabled = true) then
                        curTaxablePay := curGrossTaxable - (curSalaryArrears + curMaxPensionContrib + curOOI + curHOSP + curNonTaxable + AHLTaxable + SHATaxable)
                    else begin
                        curTaxablePay := curGrossTaxable - (curSalaryArrears + curMaxPensionContrib + curOOI + curHOSP + curNonTaxable + AHLTaxable + SHATaxable);
                        // Message('curSalaryArrears %1 curDefinedContrib %2 curMaxPensionContrib %3 Taxable pay %4 curHOSP %5 curNonTaxable %6 AHLTaxable %7 SHATaxable %8 Gross %9', curSalaryArrears, curDefinedContrib, curMaxPensionContrib, curTaxablePay, curHOSP, curNonTaxable, AHLTaxable, SHATaxable, curGrossTaxable);
                    end;
                    if CheckChukadata = true then
                        if curPensionStaff > 6665 then
                            curTaxablePay := (curGrossTaxable + curPensionStaff * 2) - (curSalaryArrears + curDefinedContrib + curMaxPensionContrib + SHATaxable + AHLTaxable);

                end else
                    if (PhDisabled = true) then
                        curTaxablePay := curGrossTaxable - (curSalaryArrears + curDefinedContrib + curPensionStaff + curOOI + curHOSP + curNonTaxable + AHLTaxable + SHATaxable)
                    else begin
                        curTaxablePay := curGrossTaxable - (curSalaryArrears + curDefinedContrib + curPensionStaff + curOOI + curHOSP + curNonTaxable + AHLTaxable + SHATaxable);
                        // Message('Defined Cont 2 %1', curDefinedContrib);
                        // Message('curSalaryArrears %1 curDefinedContrib %2 curPensionStaff %3 Taxable pay %4 curHOSP %5 curNonTaxable %6 AHLTaxable %7 SHATaxable %8 Gross %9', curSalaryArrears, curDefinedContrib, curPensionStaff, curTaxablePay, curHOSP, curNonTaxable, AHLTaxable, SHATaxable, curGrossTaxable);
                        // Message('Less contib :curSalaryArrears %1 curDefinedContrib %2 curMaxPensionContrib %3 Taxable pay %4 curHOSP %5 curNonTaxable %6 AHLTaxable %7 SHATaxable %8 Pension %9', curSalaryArrears,curDefinedContrib,curPensionStaff, curTaxablePay , curHOSP ,curNonTaxable , AHLTaxable, SHATaxable,curPensionStaff);\Message('curSalaryArrears %1 curDefinedContrib %2 curMaxPensionContrib %3 Taxable pay %4 curHOSP %5 curNonTaxable %6 AHLTaxable %7 SHATaxable %8 Gross %9', curSalaryArrears,curDefinedContrib,curPensionStaff, curTaxablePay , curHOSP ,curNonTaxable , AHLTaxable, SHATaxable,curGrossTaxable);
                    end;
                if (curBasicPay = 0) and (blnPaysPaye = true) then
                    curTaxablePay := curGrossTaxable;
                ...
                if (curReliefPersonal + curInsuranceReliefAmount) > curMaximumRelief then begin
                    curPAYE := curTaxCharged - curMaximumRelief - curReliefPersonal;
                    // message('PAYe %1', curPAYE);
                end else begin
                    curPAYE := curTaxCharged;
                    // message('2 PAYe %1', curPAYE);
                end;

                // IF NOT ((curPensionCompany - curMaxPensionContrib)>0) THEN
                // curPAYE := curPAYE + curgratuityAmnt + curTaxOnExcessPension;

                if VitalSetup."Tax on Exc Pension Reduce PAYE" = true then
                    curPAYE := curPAYE + curgratuityAmnt + curTaxOnExcessPension
                else
                    curPAYE := curPAYE + curgratuityAmnt;
                // ELSE
                //  curPAYE :=curPAYE+curgratuityAmnt;
                // Calculate Lecturers PAYE

                //IF PhDisabled THEN curPAYE := 0;
                IF curReliefPersonal > curTaxCharged THEN
                    curPAYE := 0;
                if blnPaysPaye = false then
                    curPAYE := 0;

                // HrEmployee.Get(strEmpCode);
                // if HrEmployee."Posting Group" = 'LOCUM' then begin
                //     EmpSalary.Reset();
                //     EmpSalary.SetRange(EmpSalary."Employee Code", strEmpCode);
                //     //  EmpSalary.SetRange(EmpSalary."Payroll Period", SelectedPeriod);
                //     if EmpSalary.Find('-') then
                //         if EmpSalary."Pays PAYE" = false then
                //             if EmpSalary."Pays 30% PAYE" = true then
                //                 //curPAYE :=curGrossTaxable*0.3;
                //                 curPAYE := curGrossTaxable * 0.325;
                // end;
                if HrEmployee."Employee Type" = HrEmployee."Employee Type"::Secondary then
                    curPAYE := curTaxablePay * 0.325;
                //Added DW
                // HrEmployee.Get(strEmpCode);
                // if HrEmployee."Posting Group" = 'SALARY' then begin
                //     EmpSalary.Reset();
                //     EmpSalary.SetRange(EmpSalary."Employee Code", strEmpCode);
                //     // EmpSalary.SetRange(EmpSalary."Payroll Period", SelectedPeriod);
                //     if EmpSalary.Find('-') then
                ...
                //if strEmpCode='PL055' then error('Test5'+format(curPAYE));
                //EmpSalary.Reset;
                //EmpSalary.SetRange(EmpSalary."Employee Code", strEmpCode);
                // EmpSalary.SetRange(EmpSalary."Payroll Period", SelectedPeriod);
                //if EmpSalary.Find('-') then begin
                //   if (EmpSalary."Pays PAYE" = false) and (EmpSalary."Pays 30% PAYE" = false) then curPAYE := 0;
                //end;
                //if HrEmployee.get(strEmpCode) then
                //   PhDisabled := HrEmployee.PWD;

                //if PhDisabled = true then begin
                // if curGrossTaxable > 150000 then
                // curPAYE := (curGrossTaxable - 150000) * 0.3
                //    curPAYE := fnGetEmployeePaye((curGrossTaxable - 150000))
                //else
                //    curPAYE := 0;
                //end;
                ...
                if blnGetsPAYERelief then begin
                    ...
                    if EmpSalary."Pays 30% PAYE" then begin
                        curReliefPersonal := 0;
                        curTransAmount := 0;
                    end;
                ...
                IF intSHIF_BasedOn = intSHIF_BasedOn::Gross THEN //>NHIF calculation can be based on:
                    curShif_Base_Amount := curGrossPay - curException;
                IF intSHIF_BasedOn = intSHIF_BasedOn::Basic THEN
                    curShif_Base_Amount := curBasicPay - curException;
                IF intSHIF_BasedOn = intSHIF_BasedOn::"Taxable Pay" THEN
                    curShif_Base_Amount := curTaxablePay - curException;

                if blnPaysShif then begin
                    curSHIF := fnGetEmployeeSHIF(curShif_Base_Amount);
                    curTransAmount := curSHIF;
                    SHIFEMPyee := EmployeePostingGroup."SHIF Employee Account";
                    strTransDescription := 'S.H.I.F';
                    TGroup := 'STATUTORIES';
                    TGroupOrder := 7;
                    TSubGroupOrder := 2;
                    fnUpdatePeriodTrans(strEmpCode, 'SHIF', TGroup, TGroupOrder, TSubGroupOrder, strTransDescription, curTransAmount, 0, intMonth, intYear, '', '', SelectedPeriod, Dept, SHIFEMPyee, Journalpostas::Credit, Journalpostingtype::"G/L Account", '', Coopparameters::none);

                    // /*NHIF RELIEF*/
                    // CLEAR(curTransAmount);
                    // curTransAmount := curNHIF * 0.15;
                    // strTransDescription := 'NHIF Relief';
                    // TGroup := 'TAX CALCULATIONS';
                    // TGroupOrder := 6;
                    // TSubGroupOrder := 10;
                    // fnUpdatePeriodTrans(strEmpCode, 'NHIF-R', TGroup, TGroupOrder, TSubGroupOrder, strTransDescription,
                    // curTransAmount, 0, intMonth, intYear, '', '', SelectedPeriod, Dept, '', Journalpostas::" ", Journalpostingtype::" "
                    // , '', Coopparameters::none);
                end;
                ...
                if prEmployeeTransactions.Find('-') then begin
                    ...
                    repeat
                        ...
                        if prTransactionCodes.Find('-') then begin
                            ...
                            if (prTransactionCodes."Employer Deduction") or (prTransactionCodes."Include Employer Deduction") then
                                if prTransactionCodes."Is Formula for employer" <> '' then begin
                                    ...
                                    curTransAmount := fnGetEmployerNSSF(curGrossPay, 1);
                                    if prTransactionCodes.Pension = true then begin
                                        curTransAmount := fnGetEmployerNSSF(curGrossPay, 2);
                                        VitalSetup.Reset();
                                        VitalSetup.get();
                                        if (VitalSetup."Implement New NSSF" = true) and (VitalSetup."Adjust Pension On NSSF" = true) then
                                            curTransAmount := curTransAmount - VitalSetup."NSSF Employee"
                                        else
                                            curTransAmount := curTransAmount;
                ...
                curNetPay := curGrossPay - (curDefinedContrib + curSHIF + curPAYE + curPayeArrears + curTotalDeductions + EmployeeHousingLevy);

                // curNetPay := curGrossPay - (curPAYE); //bkk
                ...
                if curGrossPay > 0 then begin
                    ...
                    PayablesAcc := EmployeePostingGroup."Net Salary Payable";
                    TGroup := 'NET PAY';
                    TGroupOrder := 9;
                    TSubGroupOrder := 0;
    ...
    begin
        if HrEmployee.Get(strEmpCode) then begin
            if HrEmployee."Contract Type" = HrEmployee."Contract Type"::Locum then
                ProratedAmt := BasicSalary
            else
    ...
    begin
        ...
        prEmpTrans.reset();
        prEmpTrans.setrange("Employee Code", EmpCode);
        prEmpTrans.setrange("Transaction Code", TCode);
        prEmpTrans.setrange("Payroll Period", dtOpenPeriod);
        ...
        prPeriodTransactions."Loan Number" := prEmpTrans."Loan Number";
        prPeriodTransactions."Bank Account No" := prSalCard."Bank Account Number";
        prPeriodTransactions."Bank Branch" := prSalCard."Branch Bank";
        prPeriodTransactions."Bank Code" := prSalCard."Main Bank";
        //Paymode
        if prSalCard.Get(EmpCode) then
    ...
    begin
        ...
        if prTransactionCodes.Find('-') then
            repeat
                ...
                prEmployeeTransactions.SetRange(prEmployeeTransactions."Period Month", intMonth);
                prEmployeeTransactions.SetRange(prEmployeeTransactions."Period Year", intYear);
                prEmployeeTransactions.SetRange(prEmployeeTransactions."No relief", false);
    ...
    begin
        VitalSetup.Get();
        SHIF := Round(curBaseAmount * (VitalSetup."SHA Deduction Percentage" / 100));
        if SHIF < VitalSetup."Minimum SHIF Deduction" then
            SHIF := VitalSetup."Minimum SHIF Deduction";
        exit(SHIF);
    end;
    ...
    var
        ...
        prEmployeeTransactions: Record "prEmployee Transactions";
        prEmployeeTransactions2: Record "prEmployee Transactions";
    ...
    begin
        ...
        if prEmployeeTransactions.Find('-') then
            repeat
                        ...
                        case prTransactionCodes."Balance Type" of
                            prTransactionCodes."balance type"::None:
                                begin
                                    ...
                                    //FOR NITA TO CARRY TRANS CODE TO NEXT PERIOD
                ...
                if (prTransactionCodes.Frequency = prTransactionCodes.Frequency::Fixed) and
                 ...
                 begin
                    ...
                    //<<< Added for NITA
                    IF prTransactionCodes."Employer Deduction" = TRUE THEN
                        WITH prEmployeeTrans DO BEGIN
                            prEmployeeTransactions2.Reset();
                            prEmployeeTransactions2.SetRange(prEmployeeTransactions2."Employee Code", prEmployeeTransactions."Employee Code");
                            prEmployeeTransactions2.SetRange(prEmployeeTransactions2."Transaction Code", prEmployeeTransactions."Transaction Code");
                            prEmployeeTransactions2.SetRange(prEmployeeTransactions2."Payroll Period", dtNewPeriod);
                            if not prEmployeeTrans.find('-') then begin
                                prEmployeeTrans.INIT();
                                prEmployeeTrans."Employee Code" := prEmployeeTransactions."Employee Code";
                                prEmployeeTrans."Transaction Code" := prEmployeeTransactions."Transaction Code";
                                prEmployeeTrans."Transaction Name" := prEmployeeTransactions."Transaction Name";
                                prEmployeeTrans.Amount := 0;
                                prEmployeeTrans."Employer Amount" := prEmployeeTransactions."Employer Amount"; //<<< NITA
                                prEmployeeTrans.Balance := 0;
                                prEmployeeTrans."Amortized Loan Total Repay Amt" := prEmployeeTransactions."Amortized Loan Total Repay Amt";
                                prEmployeeTrans."Original Amount" := prEmployeeTransactions."Original Amount";
                                prEmployeeTrans.Membership := prEmployeeTransactions.Membership;
                                prEmployeeTrans."Reference No" := prEmployeeTransactions."Reference No";
            ...
            until prEmployeeTransactions.Next() = 0;
        // prsalCard.Reset;
        // prsalCard.SetRange(prsalCard."Employee Code");
    ...
    begin
        ...
        if SalaryCard.Get(strEmpCode) then
            if EmployeePostingGroup.Get(SalaryCard."Posting Group") then begin
                //Comment This for the Time Being

                EmployeePostingGroup.TestField("Salary Account");
                EmployeePostingGroup.TestField("Income Tax Account");
                EmployeePostingGroup.TestField("Net Salary Payable");
                EmployeePostingGroup.TestField("NSSF Employer Account");
                EmployeePostingGroup.TestField("Pension Employer Acc");
                EmployeePostingGroup.TestField("Housing Levy Employee Account");
                EmployeePostingGroup.TestField("Housing Levy Employer Account");

                TaxAccount := EmployeePostingGroup."Income Tax Account";
                salariesAcc := EmployeePostingGroup."Salary Account";
                PayablesAcc := EmployeePostingGroup."Net Salary Payable";
                NSSFEMPyer := EmployeePostingGroup."NSSF Employer Account";
                NSSFEMPyee := EmployeePostingGroup."NSSF Employee Account";
                SHIFEMPyee := EmployeePostingGroup."SHIF Employee Account";
                PensionEMPyer := EmployeePostingGroup."Pension Employer Acc";
                SHIFEMPyee := EmployeePostingGroup."SHIF Employee Account";
                HLevyEMPyee := EmployeePostingGroup."Housing Levy Employee Account";
                HLevyEMPyer := EmployeePostingGroup."Housing Levy Employer Account";
                PensionEMPyer := EmployeePostingGroup."Pension Employer Acc";
            end else
    ...
    begin
        if hrEmp.Get(empNo) then
            Dis_A := hrEmp."Job Application No...";
        if Dis_A = true then
            if (Gross <= 150000) then
                Dis_A := false;
    end;

    procedure fnGetEmployerNSSF(curBaseAmount: Decimal; Tier: Integer) NSSF: Decimal
    var
    ...
    begin
        NssfTierI := 0;
        NssfTierII := 0;
        TotalNssfContribution := 0;
        IF curBaseAmount < 72000 THEN BEGIN
            IF curBaseAmount <= 8000 THEN BEGIN
                NSSF := 8000 * 0.06;
                NssfTierII := 0;
            END ELSE IF curBaseAmount > 8000 THEN BEGIN
                IF Tier = 1 THEN
                    NSSF := 8000 * 0.06;
                IF Tier = 2 THEN
                    NSSF := (curBaseAmount - 8000) * 0.06;
            END;
        END ELSE IF curBaseAmount > 72000 THEN BEGIN
            IF Tier = 1 THEN
                NSSF := 8000 * 0.06;
            IF Tier = 2 THEN
                NSSF := 64000 * 0.06;
    ...
    procedure fnGetEmployeeNSSF(curBaseAmount: Decimal; Tier: Integer) NSSF: Decimal
    var
        prNSSF: Record "PR NSSF";
    ...
    begin
        NssfTierI := 0;
        NssfTierII := 0;
        //Message('Gross amount%1', curBaseAmount);
        TotalNssfContribution := 0;
        IF curBaseAmount <= 72000 THEN BEGIN
            IF curBaseAmount <= 8000 THEN BEGIN
                NssfTierI := 8000 * 0.06;
            ...
            END ELSE IF curBaseAmount > 8000 THEN BEGIN
                IF Tier = 1 THEN
                    NssfTierI := 8000 * 0.06;
                IF Tier = 2 THEN
                    NssfTierII := (curBaseAmount - 8000) * 0.06;
            END;
        END ELSE IF curBaseAmount > 72000 THEN BEGIN
            IF Tier = 1 THEN
                NssfTierI := 8000 * 0.06;
            IF Tier = 2 THEN
                NssfTierII := 64000 * 0.06;
        END;
    ...
    local procedure fnInsertSalCard(PayrollPeriod: Date)
    var
        prsalCard3: Record "prSalary Card";
        dtNewPeriod: Date;
        intNewMonth: Integer;
        intNewYear: Integer;
    begin
        prsalCard3.Reset();
        if not (prsalCard3.Get(prsalCard."Employee Code", PayrollPeriod)) then begin // Insert the Basic salary details for the current Month
            prsalCard3.Init();
            prsalCard3."Employee Code" := prsalCard."Employee Code";
            prsalCard3."Payroll Period" := dtNewPeriod;
            prsalCard3."Basic Pay" := prsalCard."Basic Pay";
