Report 52202547 prPayrollSummary2
{
    DefaultLayout = RDLC;
    RDLCLayout = './Layouts/PayrollSummary2.rdl';
    ...
    dataset
    {
        dataitem("HR-Employee"; "HR-Employee")
        {
            ...
            column(nhifRL; nhifRL)
            {
            }
            column(HousingLevy; HousingLevy)
            {
            }
            column(nhifRLamtTotal; nhifRLamtTotal)
            {
            }
            ...
            begin
                ...
                prPeriodTransactions.SetRange(prPeriodTransactions."Payroll Period", periods);
                prPeriodTransactions.SetRange(prPeriodTransactions."Employee Code", "HR-Employee"."No.");
                prPeriodTransactions.SetRange(prPeriodTransactions."Transaction Code", 'GPAY');
                if prPeriodTransactions.Find('-') then
                    TranscAmount[1, countgpay] := prPeriodTransactions.Amount;
                ...
                prPeriodTransactions.SetRange(prPeriodTransactions."Payroll Period", periods);
                prPeriodTransactions.SetRange(prPeriodTransactions."Employee Code", "HR-Employee"."No.");
                prPeriodTransactions.SetRange(prPeriodTransactions."Transaction Code", 'HLEVY');
                if prPeriodTransactions.Find('-') then
                    HousingLevy := prPeriodTransactions.Amount;
                                ...
                                if prPeriodTransactions."Transaction Code" = 'NSSF' then begin
                                    ...
                                    if prPeriodTransactions."Transaction Code" = 'NHIF' then begin
                                        ...
                                        if prPeriodTransactions."Transaction Code" = 'NHIF-RL' then begin
                                            nhifRL := prPeriodTransactions.Amount;
                                            ;
                                            nhifRLamtTotal := (nhifRLamtTotal + (prPeriodTransactions.Amount));
                                        end else
                ...
                /////(BasicPay=0)  (GrossPay = 0) or or removed by mk
            ...
            begin
                ...
                counts := counts + 1;
                countgpay := counts;
                TransName[1, countgpay] := 'GPAY';
                Transcode[1, countgpay] := 'Gross Pay';
