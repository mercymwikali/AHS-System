Report 52202582 "Individual Payslips 2 mst"
{
    DefaultLayout = RDLC;
    RDLCLayout = './Layouts/IndividualPayslips2mst.rdl';
    ...
    dataset
    {
        dataitem("prSalary Card"; "prSalary Card")
        {
                ...
                if objEmp.Find('-') then begin
                    if objEmp."Shortcut Dimension 2 Code" <> '' then begin
                        ...
                        if Dimm.Find('-') then begin
                            dept := Dimm.Name;
                            strSection := Dimm.Name;
                        end;

                        Dimm.Reset();
                        //  Dimm.SetRange(Dimm."Dimension Code", 'CAMPUS');
                        Dimm.SetRange(Dimm.Code, objEmp."Campus Code");
                        if Dimm.Find('-') then
                            strCampus := 'Campus: ' + Dimm.Name;
                    end;
                    ...
                    strPin := objEmp."PAYE Number";
                    strJob := 'Job: ' + objEmp."Job Specification";
                    strGradeLevel := 'Grade: ' + objEmp.Grade;
                    ...
                    repeat
                        ...
                        if strGrpText <> PeriodTrans."Group Text" then begin
                            ...
                            (PeriodTrans."Group Text" <> 'NET PAY') then begin
                                ...
                                if Trans[ColumnNo, Index] = '' then begin
                                    PeriodTrans.CalcFields("Trans Desc2");
                                    Trans[ColumnNo, Index] := PeriodTrans."Trans Desc2";
                                end;
                                if (Trans[ColumnNo, Index] = '') and (PeriodTrans."Trans Desc2" = '') then
                                    Trans[ColumnNo, Index] := PeriodTrans."Transaction Code";
                                //  TransBal[ColumnNo,Index]:='.';
                            end;
                            ...
                            if PeriodTrans."Transaction Name" = '' then begin
                                PeriodTrans.CalcFields("Trans Desc2");
                                Trans[ColumnNo, Index] := PeriodTrans."Trans Desc2";
                            end;
                            if (PeriodTrans."Transaction Name" = '') and (PeriodTrans."Trans Desc2" = '') then
                                Trans[ColumnNo, Index] := PeriodTrans."Transaction Code";
            ...
            begin
                ...
                Evaluate(PeriodFilter, "prSalary Card".GetFilter("Period Filter"));
