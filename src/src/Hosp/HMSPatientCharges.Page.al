Page 52202657 "HMS Patient Charges List"
{
    PageType = List;
            ...
            repeater(Group)
            {
                ...
                field(Amount; Rec.Amount)
                {
                    ...
                    begin
                        ...
                        //     Error('Drug Amount cannot be edited');
                    end;
                }
        ...
        area(Navigation)
        {
            action("Post Open Charges")
            {
                ...
                begin
                    ...
                    if HMSCharges.Find('-') then
                        repeat
                            HMSPatCU.PostCharges(Rec."Patient No.", HMSCharges."Visit No", HMSCharges."Own Debtor");
            ...
            action("Raise Weiver Request")
            {
                ...
                begin
                    ...
                    HMSChargeWeiver."Dimension1 Code" := Rec."Shortcut Dimension 1 Code";
                    HMSChargeWeiver."Dimension2 Code" := Rec."Shortcut Dimension 2 Code";
            ...
            action("Post Drugs Returns")
            {
                ...
                trigger OnAction()
                begin
                    if Confirm('Do you really want to post Item return?', false) then begin
                        if Rec.Posted = true then
                            Error('This charge has been posted. Kindly reverse the charge');
            ...
            action("Post Reversal")
            {
                ...
                trigger OnAction()
                var
                    CuHmsProc: Codeunit "HMS Processes";
                begin
                    if UserSetUp.Get(Database.UserId) then begin
                        if UserSetUp."Can Delete Charges" then begin
                            ...
                            HMSCharges.SETRANGE(HMSCharges."Visit No", Rec."Visit No");
                            HMSCharges.SETRANGE(HMSCharges.Posted, TRUE);
                            HMSCharges.SETRANGE(HMSCharges."Posting Document No", Rec."Posting Document No");
                            IF HMSCharges.FIND('-') THEN BEGIN
                                HMSPatCU.Post_Reverse_Charge(HMSCharges."Posting Document No", HMSCharges."Visit No");
    ...
    begin
        if HmsTransCode.Get(Rec."Transaction Type") then
            if HmsTransCode."Require Doctor ID" = true then
                if Rec."Doctor ID" = '' then
                    Message('Kindly insert doctor ID column');
    end;
