Table 85199 "HMS Mothers List"
{
    // DrillDownPageID = UnknownPage39005998;
    //  LookupPageID = UnknownPage39005998;
    ...
    fields
    {
        ...
        field(50002; "Patient Type"; Option)
        {
            ...
            begin
                ...
                if "Patient Type" <> xRec."Patient Type" then
        ...
        field(50011; "ID Number"; Code[25])
        {
            ...
            begin
                ...
                if DuplicateCustomerExists() > 1 then
        ...
        field(50085; "Patient Ref. No."; Code[20])
        {
            ...
            begin
                if "Patient Type" = "patient type"::Cash then begin
                ...
                end else if "Patient Type" = "patient type"::" " then begin
                    ...
                    if "Depandant Principle Member" = '' then
                        Error('Please specify the Principal member number!');
                    benefic.SetRange(benefic."Entry No", "Patient Ref. No.");
                    if benefic.Find('-') then
    ...
    begin
        if Inpatient = false then begin
            valid := true;
            if StrLen(control) = 1 then
                valid := false;
            NewStrings := DelChr(UpperCase(control), '=', '!@#$%^&*()_+|}{":;''?>><,./-\-~`ABCDEFGHIJKLMNOPQRSTUVWXYZ');
            if StrLen(NewStrings) <= 1 then
                valid := false;
            if NewStrings = '' then
                valid := false;
            if valid = false then
                Error('Please enter the correct value to proceed');
        end;
    end;
    ...
    begin
        if Inpatient = false then begin
            valid := true;
            if StrLen(control) < mLength then
                valid := false;
            if valid = false then
                Error('Please enter the correct number of characters to proceed');
        end;
    end;
}
